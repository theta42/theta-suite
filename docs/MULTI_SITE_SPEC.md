...
>
> **Read the status table before trusting the three bullets above.** Several things this document has described as shipped since early 2026 had in fact never worked end to end — the join's LDAP import, catalog updates/deletions reaching a spoke, promotion with more than two sites, and the mesh carrying any service traffic at all. Each failure surfaced only in a note string or a swallowed exception that no test asserted on. They are fixed and covered by real multi-container tests as of 2026-08-11; the lesson worth keeping is that "verified" in this file historically meant "the happy path returned 200", not "the effect was observed".
...
| Mesh **data plane** (relay/replication actually crossing the tunnel) | **Fixed 2026-08-11** — previously shipped "at the API/automation layer" only, and the layer underneath did not exist. WireGuard runs inside the `theta-gateway` container's namespace, so the `172.24.<idx>.1` mesh IP a spoke registered was unreachable from the `sso-manager`/`theta-proxy` containers that were told to use it, and nothing listened on `:3001` there anyway. Relay routes and mesh-preferred resync pushes both pointed at a dead target. `theta-gateway` now runs userspace forwarders in both directions (`services/mesh_forwarder.js`): ingress `172.24.<own>.1:3001 → sso-manager:3001`, egress `0.0.0.0:<30000+peer> → 172.24.<peer>.1:3001`. Consumers dial the LOCAL gateway on a port derived from the peer's mesh index (`sso-manager-node/utils/mesh_route.js`) — nothing stored, nothing discovered. Verified with a real two-gateway WireGuard tunnel carrying HTTP to a far-side service (`jump-host/docker-compose.mesh-e2e.yml`). |
