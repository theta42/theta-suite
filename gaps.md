...
| A22 | sso | `network_driver.js` mapped its SSH probe with `(probe && probe.responseTimeMs) \|\| null`. A probe answering in under a millisecond reports `0`, and `0 \|\| null` is null — so every SSH service on loopback or a fast LAN reported **no** response time. Surfaced as a CI failure that hit one matrix leg and passed the other on the same commit | `??` instead of `\|\|`; regression test stubs the probe rather than racing the loopback, and was checked against the old code to confirm it catches this. Same defect class as A20 | fixed |
| A23 | sso + proxy | Every open Dependabot advisory in both repos (1 high + 5 moderate, and 2 high + 4 moderate) had gone stale. `nodemailer`'s PR had been red since it was opened — because of A19's flaky test, not the dependency: our flake was blocking a security update where nobody would look | all six rebased onto current master, re-verified and merged; both repos now report zero open alerts | fixed |
| A24 | sso | A test bound a hardcoded port (`22222`) and asserted "closed" against a port it never verified was closed | ephemeral `:0` with the port read back; the closed port is bound then released. Latent, not what was failing CI — recorded so the distinction is not lost | fixed |
