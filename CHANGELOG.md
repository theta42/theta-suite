...
### Fixes
- **CRITICAL: adding a spoke destroyed group memberships on the MASTER.** Reproduced on a live master/spoke pair and traced through the master's own audit log. `god_admin` lost its real member and `app_sso_service_account` lost two, all reset to the spoke's bootstrap defaults; the master's entire `ou=people` was torn down and re-added underneath logged-in sessions.

  One mistake, two mechanisms — the spoke **re-created** the master's entries instead of **copying** them. `ldapadd` cannot modify an entry that already exists, so every DN the container entrypoint seeds on first boot (the base DN, `ou=people`, `ou=groups`, `cn=admin`, and the five global `god_admin` / `app_sso_*` groups) was silently skipped and reported as success. Everything that *did* import got a fresh `entryUUID` and a new `entryCSN` — so once multi-provider replication came up, syncrepl found the spoke's minutes-old entries newer than the master's and propagated them **backwards**.

  A join now seeds the local directory offline — slapd stopped, bootstrap tree replaced by the master's `slapcat` dump with operational attributes intact, slapd restarted — which is how an OpenLDAP replica has always been meant to be brought up. A resync is unchanged and still additive.

- **`theta-agent <command> help` printed the argument parser's error, not documentation.** Help is hierarchical now, and there are man pages — generated by the binary from the same command registry that backs the interactive help, so the two cannot describe different CLIs.

- **The agent installer replaced the binary while the old agent was still running.** On Linux the running process stays on the previous inode, so the upgrade reported success and changed nothing. It now stops the service first, stops per-user tray units, and restarts only if it was running to begin with.

- **The agent installer put a desktop tray on every host, headless servers included.** Detection now takes a live graphical session, an enabled display manager, installed session definitions, or an X socket. `systemctl get-default` is deliberately excluded: measured on two real headless theta-suite servers, both report `graphical.target` while having none of the above.

### Added
- **`theta-agent verify`** — checks configuration and key material are usable and exits non-zero if not; the installer gates on it. A `public_key` that did not decode used to produce an agent that started, connected, and then silently rejected every signed command.
- `--tray` / `--no-tray` on the agent installer.

### Submodules Updated
- `theta-directory`: **v2.26.1 → v2.27.0** — offline LDAP replica seeding on join.
- `theta-agent`: **v2.12.0 → v2.13.0** — CLI help, man pages, installer safety, `verify`.

### Still open
Two items from the same report are **not** in this release, and remain outstanding:
- A systemd service registered through the agent appears in the Directory with no UI to see its state or control it (agreed scope: status plus start/stop/restart, and metrics over time).
- Only the stack host is offered as a jump target; any Linux host running the agent should become one automatically.
